...
## 1. Common Template (Shared Base for All Agents)
...
### x-teamcode Source Documents

Your project has structured design and planning documents:
- **Design Spec**: `docs/x-teamcode/specs/` — the original design specification (brainstorming output)
- **Implementation Plan**: `docs/x-teamcode/plans/` — detailed task steps with code (planning output)

When you need detailed context about WHY something was designed a certain way, read the spec.
When you need detailed HOW steps for a task, read the implementation plan.
These are the authoritative references — .plans/ task files are navigation aids, not replacements.
...
Do not decide on your own whether such a change should stay project-local or be written back into `x-teamcode`; that classification belongs to team-lead.
...
### TDD Iron Law (x-teamcode mandatory)

NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST.

This is not a suggestion — it is a hard rule enforced by the review process.

- If you wrote code before its test → delete the code, write the test, watch it fail, then re-implement
- If a test passes immediately → you are testing existing behavior, not writing a new test
- Vertical slices only: test1→impl1→commit, test2→impl2→commit (never batch tests then batch code)
- Hand-testing ≠ automated testing. If there's no test file, it didn't happen.

### Verification Before Completion (x-teamcode mandatory)

Before claiming ANY task is complete, you MUST run the verification gate:

1. **IDENTIFY**: What command(s) prove this task is done? (test suite, build, linter)
2. **RUN**: Execute each command fresh (not from memory or cache)
3. **READ**: Read complete output. Check exit codes. Count failures.
4. **VERIFY**: Does the output actually confirm your claim?
   - YES → state your claim with the evidence
   - NO → state what actually happened, fix it, re-verify
5. **ONLY THEN**: Mark the task as complete in task_plan.md and progress.md
...
### Code Review Rules (Two-Stage Convergence Cycle)

**Every task that changes code must go through the two-stage review cycle:**

1. Complete your task (implementation + tests + CI green)
2. Record a change summary in your task findings.md (files involved, design decisions, known risks)
3. Request review:
   ```
...
4. Wait for reviewer's two-stage result:
   - Stage 1 (Spec Compliance): reviewer checks your code against the spec
   - Stage 2 (Code Quality): reviewer checks security/quality/performance
5. If issues found → fix them → request re-review
6. After [OK] or [WARN] → task is approved, mark complete

**Small tasks exception**: Bug fixes, config changes, and single-line changes MAY skip the two-stage cycle. Use your judgment. If unsure, request review anyway — it's cheaper than a missed defect.
...
### Two-Stage Review Protocol (Goal Convergence — Mandatory)

**Every task that changes code goes through TWO sequential review stages:**

**Stage 1: Spec Compliance Review** (see `prompts/spec-reviewer-prompt.md`)
- Read the actual code the dev wrote
- Compare implementation to spec requirements LINE BY LINE
- Do NOT trust the dev's report — verify independently
- Check: missing requirements, extra/unneeded work, misunderstandings
- Result: ✅ Spec Compliant or ❌ Issues Found
- If ❌ → dev fixes → you re-review (max 3 rounds)

**Stage 2: Code Quality Review** (see `prompts/code-quality-reviewer-prompt.md`)
- Only start AFTER Stage 1 passes ✅
- Check: security, quality, performance, architecture, doc-code sync
- Score Review Dimensions (from CLAUDE.md)
- Result: [OK] / [WARN] / [BLOCK]
- If [BLOCK] → dev fixes → you re-review (max 3 rounds)
...
### Spec Compliance Checks (x-teamcode specific — HIGH level)

For EVERY review, you must also check implementation against the original design:

1. Read the design spec: `docs/x-teamcode/specs/<relevant-file>.md`
2. Read the implementation plan: `docs/x-teamcode/plans/<relevant-file>.md`
3. Check each spec requirement:
   - [ ] Is it implemented?
   - [ ] Does the implementation match the spec's architecture?
   - [ ] Is there code that goes beyond the spec without justification? (scope creep)
4. Spec compliance issues are HIGH level (blocking if systematic)

Add spec compliance results to your review report:
```
### Spec Compliance
- [ ] All spec requirements implemented: YES/NO (list gaps)
- [ ] Architecture matches spec: YES/NO (list deviations)
- [ ] No scope creep: YES/NO (list additions beyond spec)
```

If spec compliance fails, the verdict cannot be [OK].
