...
- team-lead decides whether a workflow improvement is project-local or should be written back into `x-teamcode`
...
| Design Spec | docs/x-teamcode/specs/ | team-lead (created during planning phase) |
| Implementation Plan | docs/x-teamcode/plans/ | team-lead (created during bridging phase) |
...
| Template sync | Durable workflow improvement found | Update `x-teamcode` source first, then sync project docs |
...
| Plan deviation | Dev finds plan doesn't match reality | Record in findings.md + notify team-lead. Lead updates plan. |
...
- **Template-level**: future teams should inherit the change, so update `x-teamcode` first
...
> Source Spec: docs/x-teamcode/specs/<file>.md
> Source Plan: docs/x-teamcode/plans/<file>.md
...
| Implementation Plan | docs/x-teamcode/plans/<file>.md | Detailed implementation steps |
...
## Phase State (phase-state.md)

x-teamcode specific file for tracking the current project phase through the planning-bridging-executing lifecycle.
...
- <date>: planning — spec written: docs/x-teamcode/specs/<file>.md
- <date>: planning — plan written: docs/x-teamcode/plans/<file>.md
